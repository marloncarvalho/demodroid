...
public class EntityManagerSQLiteImpl implements EntityManager {
	...
	private static Map<Class<?>, MappedEntity> cached = new WeakHashMap<Class<?>, MappedEntity>();
	...
	public <T> T find(Class<T> clazz, Object primaryKey) throws SystemException {
		...
		Cursor cursor = databaseHelper.getWritableDatabase().query(mappedEntity.getTableName(), null, "id=?", new String[] { primaryKey.toString() }, null, null, null);
